...
def preprocessing(disease):
    ...
    X = disease[['체온','수축기 혈압','이완기 혈압','호흡 곤란','간헐성 경련','설사','기침','출혈','통증','만지면 아프다','무감각','마비','현기증','졸도','말이 어눌해졌다','시력이 흐려짐','발열','고혈압','저혈압']]
...
with tab1:
    ...
    with col110:
        st.info('출동일')
    with col111:
        input_date = st.date_input('출동일', label_visibility='collapsed')
    with col112:
        st.info('출동시간')
    with col113:
        input_time = st.time_input('출동시간', label_visibility='collapsed')
    ...
    with col122:
        st.info('나이')
    with col123:
        age = st.number_input('나이', min_value=0, max_value=130, label_visibility='collapsed')
    with col124:
        st.info('성별')
    with col125:
        patient_s = st.radio('성별', ['남', '여'], 
    ...
    with col132:
        st.info('환자 위치')
    with col133:
        location = st.text_input('환자 위치', value='대구광역시', label_visibility='collapsed')
    ...
    with col140:
        st.info('수축기 혈압')
    with col141:
        high_blood = st.slider('수축기 혈압', 10, 200, label_visibility='collapsed') # 140이상 고혈압, 90이하 저혈압
    with col142:
        st.info('이완기 혈압')
    with col143:
    ...
    col150, col151, col152, col153, col154, col155, col156, col157 = st.columns(8) # col 나누기
    with col150:
        st.error("증상 체크")
    with col151:
    ...
    col160, col161, _ = st.columns([0.1, 0.2, 0.3]) # col 나누기
    with col160:
        st.error("중증 질환 여부")
    with col161:
        special_yn = st.selectbox('증증 질환 선택', ('증증 질환 선택', '중증 질환 아님', '중증 질환 예측'), label_visibility='collapsed')


    ##-------------------------------------------------------------------------------------    
    ## -------------------- ▼ 1-7그룹 중증 질환 선택 또는 예측 결과 표시 cols 구성 ▼ --------------------
    
    _, col170 = st.columns([0.05, 0.95]) # col 나누기
    with col170: 
    ...
    with st.form(key='tab2_first'):
        ...
        ## 완료시간 시간표시 cols 구성
        col190, col191, _ = st.columns([0.1, 0.2, 0.3]) # col 나누기
        with col190:
            st.info('완료시간')
        with col191:
            end_time = st.time_input('완료시간', label_visibility='collapsed')
...
with tab2:
    ...
    ## -------------------------------------------------------------------
    ## -------------------- ▼ 2-1그룹 통계 조회 기간 선택하기 ▼ --------------------
    col200, col201, col202 = st.columns([0.3, 0.2, 0.1])
    with col200:
        slider_data = st.slider('날짜', min_value=min_date, max_value=max_date, 
    ...
    day_list_df = data[(data['datetime'] >= slider_data[0]) & (data['datetime'] <= slider_data[1])]
    ...
    col220, col221, col222 = st.columns(3)
    with col220:

        group_day_time = day_list_df.groupby(['datetime'], as_index=False)['이송 시간'].mean()
        group_day_time = group_day_time.rename(columns={'datetime': '일별', '이송 시간': '이송시간'})
        st.line_chart(data=group_day_time, x='일별', y='이송시간', use_container_width=True)

    with col221:

        group_week_time = week_list_df.groupby(['주별'], as_index=False)['이송 시간'].mean()
        group_week_time = group_week_time.rename(columns={'이송 시간': '이송시간'})
        st.line_chart(data=group_week_time, x='주별', y='이송시간', use_container_width=True)

    with col222:

        group_month_time = month_list_df.groupby(['월별'], as_index=False)['이송 시간'].mean()
        group_month_time = group_month_time.rename(columns={'이송 시간': '이송시간'})
        st.line_chart(data=group_month_time, x='월별', y='이송시간', use_container_width=True)
    ...
    dispatch_count = re_select_df.groupby(select_bins, as_index=False)['ID'].count()
    dispatch_count = dispatch_count.rename(columns={'ID': '출동 건수'})
    dispatch_count = dispatch_count.sort_values(select_bins)

    st.bar_chart(data=dispatch_count, x=select_bins, y='출동 건수')
    ...
    with col240: # 성별 통계
        ...
        group_day_disease = group_day_disease.rename(columns={'ID': '성별 통계'})

        fig = px.pie(group_day_disease, values='성별 통계', names='성별', title='성별 통계', hole=.3)
    ...
    with col241: # 중증질환별 통계
        ...
        group_day_disease = group_day_disease.rename(columns={'ID': '중증질환 통계'})

        fig = px.pie(group_day_disease, values='중증질환 통계', names='중증질환', title='중증질환 통계', hole=.3)
        fig.update_traces(textposition='inside', textinfo='percent+label+value')
        fig.update_layout(font=dict(size=16))
        st.plotly_chart(fig)
    ...
    with col242:  # 나이대별 통계
        ...
        group_day_disease = group_day_disease.rename(columns={'ID': '나이대 통계'})

        fig = px.pie(group_day_disease, values='나이대 통계', names='나이대', title='나이대 통계', hole=.3)
        fig.update_traces(textposition='inside', textinfo='percent+label+value')
        fig.update_layout(font=dict(size=16))
        st.plotly_chart(fig)
    ...
    col250, col251 = st.columns(2)
    
    with col250: # 일간 통계

        group_day_disease = day_list_df.groupby(by=['datetime', '중증질환'], as_index=False)['ID'].count()
        group_day_disease = group_day_disease.rename(columns={'datetime': '일별', 'ID': '일별 통계'})

        fig = px.pie(group_day_disease, values='일별 통계', names='중증질환', title='일별 통계', hole=.3)
        fig.update_traces(textposition='inside', textinfo='percent+label+value')
        fig.update_layout(font=dict(size=16))
        st.plotly_chart(fig)
    ...
    with col251: # 주간 통계
        ...
        group_day_disease = week_list_df.groupby(by=['주별', '중증질환'], as_index=False)['ID'].count()
        group_day_disease = group_day_disease.rename(columns={'ID': '주별 통계'})

        fig = px.pie(group_day_disease, values='주별 통계', names='중증질환', title='주별 통계', hole=.3)
        fig.update_traces(textposition='inside', textinfo='percent+label+value')
        fig.update_layout(font=dict(size=16))
        st.plotly_chart(fig)
